...
import { List, X, User, Ticket, CaretDown } from '@phosphor-icons/react'
...
export function Header() {
  const { isVerified, remaining, setUserId } = useTrial()
  ...
  const handleRegister = async () => {
    ...
    const result = await register({ account: regAccount, password: regPassword, nickname: regNickname, phone: regPhone, email: regEmail })
    if (result.success) {
      setRegSuccess(true)
  ...
  const handleLogout = () => { logout(); setUser(null); setUserId(null) }
  ...
  return (
    <>
      {/* Fixed navy nav — classic, like the brochure */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-center gap-2 sm:gap-4 lg:gap-8 h-14">
            ...
            <div className="hidden lg:flex items-center gap-2 ml-2">
                  ...
                  {isAdmin() && (
                    <div className="relative">
                      <button
                        ...
                        className="flex items-center gap-1 px-3 py-1.5 text-xs text-paper/65 hover:text-copper transition-colors tracking-wider"
                      >
                        管理 <CaretDown weight="bold" className={`w-3 h-3 transition-transform duration-200 ${adminOpen ? 'rotate-180' : ''}`} />
                      </button>
                      <AnimatePresence>
                        {adminOpen && (
                          <motion.div
                            ...
                            className="absolute top-full mt-1 right-0 w-40 bg-navy-light border border-copper/20 rounded shadow-lg overflow-hidden z-50"
                          >
                            <Link href="/admin/articles" onClick={() => setAdminOpen(false)} className="block px-4 py-2.5 text-xs text-paper/65 hover:text-copper hover:bg-navy/50 transition-colors">文章管理</Link>
                            <Link href="/admin/videos" onClick={() => setAdminOpen(false)} className="block px-4 py-2.5 text-xs text-paper/65 hover:text-copper hover:bg-navy/50 transition-colors">视频管理</Link>
                            <Link href="/admin/trial" onClick={() => setAdminOpen(false)} className="block px-4 py-2.5 text-xs text-paper/65 hover:text-copper hover:bg-navy/50 transition-colors">体验码管理</Link>
                          </motion.div>
                        )}
                      </AnimatePresence>
                    </div>
              ...
              {hasTrialRemaining ? (
                <div className="flex items-center gap-1.5 bg-copper/15 rounded-full px-3 py-1.5">
                  <Ticket weight="bold" className="w-3.5 h-3.5 text-copper" />
                  <span className="text-copper text-xs font-medium">{remaining!.qiaoxi + remaining!.qiaoyuan + remaining!.cxr}</span>
                </div>
              ) : user ? (
                <Button variant="copper" size="sm" onClick={() => setShowTrialModal(true)}>免费体验</Button>
              ) : (
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            animate="open"
            exit="closed"
            className="fixed inset-0 z-40 bg-navy flex flex-col items-center justify-center gap-2"
          >
            {navItems.map((item) => (
            ...
            <motion.div variants={itemVariants} className="mt-8 flex flex-col items-center gap-3">
              ...
              {hasTrialRemaining ? (
                <div className="flex items-center gap-2 bg-copper/15 rounded-full px-4 py-2">
                  <Ticket weight="bold" className="w-4 h-4 text-copper" />
                  <span className="text-copper font-medium">{remaining!.qiaoxi + remaining!.qiaoyuan + remaining!.cxr} 次</span>
                </div>
              ) : user ? (
                <Button variant="copper" size="md" onClick={() => { setShowTrialModal(true); setIsMobileMenuOpen(false); }}>免费体验</Button>
              ) : null}
            </motion.div>
          </motion.div>
        )}
      ...
      {/* Trial code modal */}
      <TrialCodeModal isOpen={showTrialModal} onClose={() => setShowTrialModal(false)} />
                ...
                <div className="space-y-4">
                  {regSuccess ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-slate-light text-sm mt-1">正在跳转到登录页面...</p>
                    </div>
                  ) : (
                    <>
                      <div>
                        <label className="block text-xs font-medium text-slate mb-1.5 tracking-wider">账号 *（手机号或邮箱）</label>
                        <input type="text" value={regAccount} onChange={(e) => setRegAccount(e.target.value)}
                          className="w-full px-4 py-2.5 border border-navy/10 focus:ring-1 focus:ring-copper/30 focus:border-copper outline-none transition-all text-sm"
                          placeholder="请输入手机号或邮箱" autoFocus />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-slate mb-1.5 tracking-wider">密码 *</label>
