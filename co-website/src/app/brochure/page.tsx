        ...
        <div className="max-w-[800px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            {[
              ...
              { name: '契审通', sub: 'AI商业合同决策系统', desc: '上传合同，系统直接告诉你&ldquo;签/改/拖/退&rdquo;——<strong>国内唯一把决策写进答案的合同审查产品，不是列风险清单让你自己判断。</strong>55,088条法规本地检索，6路独立审计。', featured: false },
              { name: '成章通', sub: 'AI公文工作台', desc: '小微企业老板经常遇到一种窘境：需要给税局写个请示，不会写；需要给工商局报个说明，格式不对；需要发个内部通知，连个合格的秘书都没有。成章通就是为这种窘境设计的。与前三个产品不同，成章通内核是智能Agent：锁定文种、结构铁律、四角色评审（党办/政研/纪检/基层），确保公文合规不踩雷。<strong>22种法定公文全覆盖，GB/T 9704国标排版，连格式都替你搞定。</strong>你只需要说清楚要写什么，剩下的交给成章通。', featured: false },
            ].map((p) => (
              <div key={p.name}
